...
$(function () {
    $("#ddlULD").change(function () {
        ...
        if (selectedULDNo == "Select") {
            $('#spnValMsg').text('Please select an ULD').css('color', 'red');
            $("#txtScannedID").prop('disabled', true);
        }
        else {
    ...
    //    if ($(this).is(":checked")) {
    //        $("#btnOffload").prop('disabled', true);
    //    }
    //    else {
    //        $("#btnOffload").prop('disabled', false);
    //    }
    //});
});
...
function GetShipmentInfoForAWB() {
    ...
    var scanedCode = $('#txtScannedID').val();
    ...
    if (scanedCode == '') {
        //   $('#spnValMsg').text("Please scan/ enter No.").css('color', 'red');
        return;
    }
    ...
    var connectionStatus = navigator.onLine ? 'online' : 'offline'
    var errmsg = "";
    //'<Root><flightSeqNo>'+FlightSeqNo+'</flightSeqNo><ScanCode>'+scanedCode+'</ScanCode><Offpoint>'+$('#ddlOffPoint').find('option:selected').text()+'</Offpoint><AirportCity>'+AirportCity+'</AirportCity></Root>'
    //var inputXML = '<Root><flightSeqNo>' + FlightSeqNo + '</flightSeqNo><ScanCode>' + scanedCode + '</ScanCode><Offpoint>' + $('#ddlOffPoint').find('option:selected').text() + '</Offpoint><AirportCity>' + AirportCity + '</AirportCity></Root>';
    var inputXML = '<Root><FlightSeqNo>' + FlightSeqNo + '</FlightSeqNo><ULDSeqNo>' + $('#ddlULD').find('option:selected').val() + '</ULDSeqNo><ScanCode>' + scanedCode + '</ScanCode><AirportCity>' + AirportCity + '</AirportCity><UserID>' + UserId + '</UserID></Root>';

    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            //url: GHAExportFlightserviceURL + "UnitizationPendingAWBDetails",
            url: GHAExportFlightserviceURL + "Offload_Search_V4",
            data: JSON.stringify({ 'InputXML': inputXML }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            beforeSend: function doStuff() {
            ...
            success: function (response) {
                ...
                if (isAuto == 1 && sts != 'S' && shipmentId != undefined && awbNo != undefined) {
...
function SaveAWBforULDDetails() {
    if ($("#txtScannedID").val() == '') {
        $('#spnValMsg').text('Please enter Scan ID').css('color', 'red');
        return;
    } else {
        $('#spnValMsg').text('');
    }
    ...
    //var inputXML = '<Root><FlightSeqNo>' + FlightSeqNo + '</FlightSeqNo><ULDSeqNo>' + $('#ddlULD').find('option:selected').val() + '</ULDSeqNo><Type>' + ULDBulkType + '</Type><Offpoint>' + $('#ddlOffPoint').find('option:selected').text() + '</Offpoint><AirportCity>' + AirportCity + '</AirportCity><UserID>' + UserId + '</UserID><ULDType>' + ULDType + '</ULDType><ULDNumber>' + ULDNumber + '</ULDNumber><ULDOwner>' + ULDOwner + '</ULDOwner><AWBId>-1</AWBId><ShipmentId>' + shipmentId + '</ShipmentId><AWBNo>' + awbNo + '</AWBNo><NOP>' + nop + '</NOP><Weight>' + weight + '</Weight><Volume>-1</Volume><HAWBRowId>-1</HAWBRowId></Root>';
    var inputXML = '<Root><FlightSeqNo>' + FlightSeqNo + '</FlightSeqNo><ULDSeqNo>' + $('#ddlULD').find('option:selected').val() + '</ULDSeqNo><ManifestSeqNo>' + ManifestSeqNo + '</ManifestSeqNo><Type>' + ULDBulkType + '</Type><Offpoint>' + $('#ddlOffPoint').find('option:selected').text() + '</Offpoint><AirportCity>' + AirportCity + '</AirportCity><UserID>' + UserId + '</UserID><ULDType>' + ULDType + '</ULDType><ULDNumber>' + ULDNumber + '</ULDNumber><ULDOwner>' + ULDOwner + '</ULDOwner><ShipmentId>' + ShipmentId + '</ShipmentId><NOP>' + NOP + '</NOP><Weight>' + Weight + '</Weight><Volume>' + VOLUME + '</Volume><Reason></Reason><Remark></Remark><ScanCode>' + $('#txtScannedID').val() + '</ScanCode></Root>';
    console.log(inputXML);
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: "POST",
            url: GHAExportFlightserviceURL + "Offload_Save_V4",
            //data: JSON.stringify({
