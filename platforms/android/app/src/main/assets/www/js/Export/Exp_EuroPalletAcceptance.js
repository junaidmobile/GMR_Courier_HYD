...
function GetVCTDetailsForTDGAcceptance(clearMsg, overwriteMsg) {
    ...
    Door = $('#txtVCTNo').val();
    window.localStorage.setItem("Door", Door);
    var inputXML = '<Root><VCTNo>' + $('#txtVCTNo').val() + '</VCTNo><AirportCity>' + AirportCity + '</AirportCity><UserId>' + UserID + '</UserId></Root>'


    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAExportFlightserviceURL + "HHTGetExportVCTDetails",
            ...
            success: function (response) {
                ...
                if (str != null && str != "") {
                    ...
                    $(xmlDoc).find('Table2').each(function (index) {
                        TotalVCTPieces = $(this).find('TotalPiece').text();
                        TotalVCTGrWt = $(this).find('TotalWeight').text();
                        TotalAcceptedPieces = $(this).find('AcceptedNOP').text();
                        TotalRemaingingPieces = TotalVCTPieces - TotalAcceptedPieces;
                        DockInStatus = $(this).find('IsDockInDone').text();
                        DockOutStatus = $(this).find('IsDockOutDone').text();
                        AWBCount = $(this).find('AWBCount').text();
                        Door = $(this).find('Door').text();

                        window.localStorage.setItem("TotalWT", TotalVCTGrWt);
                        window.localStorage.setItem("RemaingPcs", TotalRemaingingPieces);
                        ...
                        if (DockInStatus == 'true' || DockOutStatus == 'true') {
                            $('#ddlDoor').empty();
                            var newOption = $('<option></option>');
                            newOption.val(0).text(Door);
                            newOption.appendTo('#ddlDoor');
                        }
                    });
                    $(xmlDoc).find('Table').each(function (index) {
                        var status = $(this).find('Status').text();
                        var strmsg = $(this).find('StrMessage').text();
                        if (status == "E") {
                            $.alert(strmsg);
                        }
                    });
                    ...
                    $(xmlDoc).find('Table1').each(function (index) {
                        ...
                        if (DockInStatus == 'false' || DockOutStatus == 'false') {
                            if (text != "Select") {
                                var newOption = $('<option></option>');
...
function UpdateVCTStatus(D_Status) {
    ...
    var isChecked = $("#chkSealed").is(":checked");
    var isSealed = isChecked ? 1 : 0;
    //if ($('#txtDoor').val() == "") {
    ...
    if (D_Status == 'I') {
        if ($("#ddlDoor option:selected").text() == "Select") {
            $('#spnErrormsg').text('Please enter door No.').css('color', 'red');
            $('#txtDoor').focus();
            return;
        } else {
            var txtDoorNo = $("#ddlDoor option:selected").text();
            $('#spnErrormsg').text('');
    ...
    var inputXML = '<Root><DockInOut>' + D_Status + '</DockInOut><TokenNo>' + txtVCTNo + '</TokenNo><DockInDateTime>' + currentDateTime + '</DockInDateTime><IsTruckSealed>' + isSealed + '</IsTruckSealed><Door>' + txtDoorNo + '</Door><CompanyCode>' + CompanyCode + '</CompanyCode><UserId>' + UserID + '</UserId><AirportCity>' + AirportCity + '</AirportCity><Culture>en-us</Culture></Root>';
    console.log(inputXML);

    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAExportFlightserviceURL + "HHT_VCT_DockInOut_V4",
            data: JSON.stringify({
                'InputXML': inputXML
            }),
            contentType: "application/json; charset=utf-8",
            ...
            success: function (response) {
                ...
                $(xmlDoc).find('Table').each(function () {
                    ...
                    if (Status == 'E') {
                        $('#spnErrormsg').text($(this).find('StrMessage').text()).css('color', 'red');
                    } else {
                        $('#spnErrormsg').text($(this).find('StrMessage').text()).css('color', 'green');
...
function goToTDGAcceptanceAWB() {
    ...
    if ($("#ddlDoor option:selected").text() == "Select") {
        ...
        txtDoorNo = $("#ddlDoor option:selected").text();
        $('#spnErrormsg').text('');

    }
    window.localStorage.setItem("VCTNo", $("#txtVCTNo").val());
    window.localStorage.setItem("Door", txtDoorNo);
